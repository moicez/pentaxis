/*
 * PentaxisArmIKSolver.cpp
 *
 *  Created on: Oct 29, 2013
 *      Author: raha, Moises Estrada
 */

#include "PentaxisArmIKSolver.h"
...
bool PentaxisArmIKSolver::getCount(int &count,
                                 const int &max_count,
                                 const int &min_count)
{
  if(count > 0)
                                 ...
                                 KDL::JntArray &q_out)
{
  ...
  pentaxis_arm_ik_.CartToJnt(q_init, p_in, solution_ik);
...
int PentaxisArmIKSolver::CartToJntSearch(const KDL::JntArray& q_in,
                                       const KDL::Frame& p_in,
                                       KDL::JntArray &q_out,
                                       const double &timeout)
{
  ...
  if(loop_time >= timeout) {
    ROS_INFO("IK Timed out in %f seconds - DEBUG",timeout);
    return TIMED_OUT;
  } else {
